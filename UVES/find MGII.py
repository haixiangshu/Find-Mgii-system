...
plt.plot(wavenew, fluxnew, 'k', label='Flux')
plt.plot(wavenew, errornew, 'r', label='Error')
plt.plot(wavenew, continuumnew, 'g', label='Continuum')
...
def Find_peaks(normalized_fluxnew, final_loc, final_locrow):
    """
    methods to find peaks in the normalized flux
    there are 72 pixels for each wavelength so we choose 576 pixels sumadd
    """

    chosenflux = np.empty(0)
    final_locrowi = np.empty(0)
    peakcandi = np.zeros((len(final_loc)))
    for i in range(len(final_loc)):
        num = final_loc[i]
        peakcandi[i] = normalized_fluxnew[num]
    peaks, _ = find_peaks(peakcandi, height=None, threshold=None, distance=(2803-2796)*72, prominence=0.05, width=None, wlen=None, rel_height=None, plateau_size=None)
    for j in range(len(peaks)):
        num1 = peaks[j]
        chosenflux = np.append(chosenflux, final_loc[num1-(15*72):num1+(15*72)])
        final_locrowi = np.append(final_locrowi, final_locrow[num1-(15*72):num1+(15*72)])
    final_loc = np.unique(chosenflux)
    final_locrow = np.unique(final_locrowi)
...
m1, m2, w1, w2, c1, c2 = parameter(location1, location2, normalized_fluxnew, wavenew, z)
...
def suit_system(location1, location2, wavelength, z):
    ...
    final_loc1 = []
    final_locrow1 = []
    final_loc, final_locrow = Find_peaks(normalized_fluxnew, final_loc, final_locrow)
    system, final_loc1, final_locrow1, d = split_candidates(final_loc, final_locrow, final_loc1, final_locrow1, z, system)
    system = np.array(system)
...
x_array, y_array_2gauss, parameterlist, coveriancelist, gauss_peak_list, system = double_Gaussian_profile(location1, location2, normalized_fluxnew, wavenew, z)
region = []
num1 = 0
cal = 0
...
ax1.plot(x_array[0, 94], y_array_2gauss[0, 94], "r", label='raw')
ax1.plot(x_array[0, 94], gauss_peak_list[0, 94], "b", label='gaussian fitting')
...
equw = calcuequivalent_width(x_array[113:125], parameterlist[6*(nset-1):6*nset])
print("equw=", equw)
print("zmgii=", zmgii)
