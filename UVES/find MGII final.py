...
def Find_peaks(normalized_fluxnew, wavelength, final_loc, final_locrow):
    ...
    for j in range(len(peaks)):
        num1 = peaks[j]
        final_loci = np.append(final_loci, np.arange(num1-(15*72), num1+(15*72)))
        final_locrowi = np.append(final_locrowi, wavelength[num1-(15*72):num1+(15*72)])
    final_loc = np.unique(final_loci)
...
def suit_system(location1, location2, wavelength, z):
    ...
    final_loc1 = []
    final_locrow1 = []
    final_loc, final_locrow = Find_peaks(normalized_fluxnew, wavenew, final_loc, final_locrow)
...
def double_Gaussian_profile(location1, location2, flux, wavelength, z):
    ...
    for n in range(len(system)-1):
        ...
        popt_2gauss, pcov_2gauss = curve_fit(_2gaussian, x_array[cal: num1], y_array_2gauss[cal: num1], maxfev = 100000000, p0=[amp1, cen1, sigma1, amp2, cen2, sigma2])
...
ax1.plot(x_array[31270:32604], y_array_2gauss[31270:32604], "r", label='raw')
ax1.plot(x_array[31270:32604], gauss_peak_list[31270:32604], "b", label='gaussian fitting')
