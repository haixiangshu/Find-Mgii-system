...
def double_Gaussian_profile(location1, location2, flux, wavelength, z):
    ...
    for n in range(len(system)-1):
        ...
        popt_2gauss, pcov_2gauss = curve_fit(_2gaussian, x_array[cal: num1], y_array_2gauss[cal: num1], maxfev = 800000, p0=[amp1, cen1, sigma1, amp2, cen2, sigma2])
...
ax1.plot(x_array[89: 102], y_array_2gauss[89: 102], "r")
ax1.plot(x_array[89: 102], gauss_peak_list[89: 102], "b")
plt.show()
def integration(x_array, parameterlist):
    ...
    x1 = np.linspace(min(x_array), max(x_array), 100)
    _2gauss = (_2gaussian(x1, amp1, cen1, sigma1, amp2, cen2, sigma2)-1)
    ...
    sum = (up - down) / 2 * np.dot(wi, _2gauss(xi))
    return sum
...
def calcuequivalent_width(x_array, parameterlist):
    ...
    return equw
zmgii = calcuZ(parameterlist[6*4:6*5])
equw = calcuequivalent_width(x_array[89:102], parameterlist[6*4:6*5])
print(equw, zmgii)
