...
def Criterion_MGII(flux, noise, continuum_flux, wavelength, z):
    ...
    for ii in range(len(flux)):
        ...
        SNR = signal/noise[ii]
        location1[ii] = np.where(6 > SNR > 2 and wavemax > observe_wave[ii], observe_wave[ii], 0)
        location2[ii] = np.where(6 > SNR > 4 and wavemax > observe_wave[ii], observe_wave[ii], 0)
...
def split_candidates(final_loc, final_locrow, final_loc1, final_locrow1, z, system):
    ...
    for k in range(len(final_loc)-1):
        if k > 0:
...
def double_Gaussian_profile(location1, location2, flux, wavelength, z):
    ...
    for k in range(len(system)-1):
        ...
        n = len(x_array)
        center = sum(x_array) / n
        mean = sum(x_array*y_array_2gauss) / n
        sigma = sum(y_array_2gauss * (x_array - mean) ** 2) / n
    # Define the Gaussian function
        ...
        popt_2gauss, pcov_2gauss = curve_fit(_2gaussian, x_array, y_array_2gauss, maxfev = 800000, p0=[amp1, cen1, sigma1, amp2, cen2, sigma2])
        ...
        return x_array, y_array_2gauss, gauss_peak_1, gauss_peak_2, gauss_peak_main, popt_2gauss, pcov_2gauss

x_array, y_array_2gauss, gauss_peak_1, gauss_peak_2,gauss_peak_main, popt_2gauss, pcov_2gauss = double_Gaussian_profile(location1, location2, normalized_flux, wave1, z)
print(x_array, y_array_2gauss, gauss_peak_1, gauss_peak_2, gauss_peak_main, popt_2gauss, pcov_2gauss)

fig = plt.figure()
ax1 = fig.add_subplot(1, 1, 1)

ax1.plot(x_array, y_array_2gauss, "r")
ax1.plot(x_array, gauss_peak_main, "b")
plt.show()
