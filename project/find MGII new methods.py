...
#normalized_flux, normalized_inv = gaussian_filter(normalized_flux, normalized_inv)
...
Ws = [Wa, Wb, Wc, Wd]
# %%
info_list = [39633136446866947, 'guadalupe', 'main', 'dark', 9920]
# %%
targ_id, release, survey, program, pix = info_list
path = '/home/haixiangshu/Documents/global/cfs/cdirs/desi/spectro/redux/%s/healpix/%s/%s' % (release, survey, program)
healpix_loc = path + '/' + str(pix)[0:2] + '/' + str(pix)
spectra = healpix_loc + '/coadd-%s-%s-%s.fits' % (survey, program, pix)
sp_petal = Table.read(spectra, hdu="FIBERMAP")[50:70]
print(sp_petal)
...
#plt.fill_between(10 ** wave_initial * (1 + z), 3, where=(10 ** wave_initial * (1 + z)>5030)&(10 ** wave_initial * (1 + z)<5070), color='grey', alpha=0.5)
...
# %%
plt.figure(figsize=(12, 6))
#plt.fill_between(10 ** wave_initial * (1 + z), 2.5, where=(10 ** wave_initial * (1 + z)>5030)&(10 ** wave_initial * (1 + z)<5070), color='grey', alpha=0.5)
...
def search_window(flux, wavelength, noise, continuum, normalized_flux, normalized_inv, z):
    ...
    for i in range(len(flux)):
        ...
        if wavelength[i] > 3629:
            nummin = i
            break
    for i in range(len(flux)):
        if wavelength[i] > 7500:
...
def Criterion_MGII(normalized_flux, normalized_inv, wavelength, z):
    ...
    for ii in range(len(normalized_flux)):
        if (ii + 3) <= (len(normalized_flux)-1):
            signal = np.absolute(normalized_flux[ii]) + np.absolute(normalized_flux[ii+1]) + np.absolute(normalized_flux[ii+2]) + np.absolute(normalized_flux[ii+3])
            ...
            if SNR > 4 and wavemax > observe_wave[ii]:
...
location1, location2 = Criterion_MGII(normalized_flux, normalized_inv, wave_initial, z)
...
def suit_system(location1, location2, wavelength, z):
    ...
    return final_loc, final_locrow

def double_Gaussian_profile(location1, location2, flux, wavelength, inv, z):
    #each n repesent one possible mgii system
    final_loc, final_locrow = suit_system(location1, location2, wavelength, z)
    peaks = Find_peaks(normalized_flux, final_loc)
    inv = 1 / inv ** 0.5
    xfinaldata = np.empty(0)
    yfinaldata = np.empty(0)
    noifinaldata = np.empty(0)
    parameterlist = []
    coveriancelist = []
    gauss_peak_list = []
    gaps = []
    for k in range(len(peaks)):
        ...
        if k <= len(peaks)-2:
            ...
            xdata = np.empty(0)
            location = np.empty(0)
            location = np.append(location, np.arange(num1 - (8 * 3), num1 + (8 * 3)))
            location = np.append(location, np.arange(num2 - (8 * 3), num2 + (8 * 3)))
            xdata = np.append(xdata, wavelength[num1 - (8 * 3): num1 + (8 * 3)])
            xdata = np.append(xdata, wavelength[num2 - (8 * 3): num2 + (8 * 3)])
            ...
            noifinaldata = np.append(noifinaldata, noi_array)
    return xfinaldata, yfinaldata, noifinaldata, parameterlist, coveriancelist, gauss_peak_list, gaps


x_array, y_array_2gauss, noi_array, parameterlist, coveriancelist, gauss_peak_list, system = double_Gaussian_profile(location1, location2, normalized_flux, wave_initial, normalized_inv, z)
region = []
num1 = 0
cal = 0

for o in range(len(system)):
    cal = num1
    num1 += system[o]
...
y_array_2gauss = -(np.array(y_array_2gauss) - 1) + 1
gauss_peak_list = -(np.array(gauss_peak_list) - 1) + 1

#for every system you can unpack list "system" to plot each system, each region is printed here
fig = plt.figure()
ax1 = fig.add_subplot(1, 1, 1)
ax1.step(x_array[cal1:cal2], y_array_2gauss[cal1:cal2], linewidth=0.512, color='r', label='raw')
#ax1.plot(x_array[cal1:cal2], y_array_2gauss[cal1:cal2], "r", label='raw')
ax1.plot(x_array[cal1:cal2], gauss_peak_list[cal1:cal2], "b", label='gaussian fitting')
ax1.plot(x_array[cal1:cal2], noi_array[cal1:cal2], "g", label='noise')
...
equw = calcuequivalent_width(x_array[cal1:cal2], parameterlist[6*(nset-1):6*nset])
print("equw=", equw)
print("zmgii=", zmgii)
