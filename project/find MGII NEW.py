    ...
    for k in range(len(final_loc)):
        if k > 0:
            if np.max(final_locrow) - (np.min(final_locrow) + 5) <= waverangemax:
                # 5 is related to line separation of the absorbers
...
ax1.plot(x_array[136:145], y_array_2gauss[136:145], "r", label='raw')
ax1.plot(x_array[136:145], gauss_peak_list[136:145], "b", label='gaussian fitting')
